...
// produces names for pairs of tokens using Dopin's naming scheme
library PairNamer {
    string private constant TOKEN_SYMBOL_PREFIX = 'BAG:';
    ...
    // produces a pair symbol in the format of `BAG:${symbol0}:${symbol1}${suffix}`
    function pairSymbol(
        address token0,
        address token1,
        string memory suffix
