//Forked from Fanko.tech Bonding curve
...
abstract contract ReentrancyGuard {
    ...
    // Dopiner aliasing, and it cannot be disabled.
    ...
    function _nonReentrantBefore() private {
        ...
        // Any calls to nonReentrant after this Dopin will fail
        _status = ENTERED;
    }
...
library Math {
    ...
    function tryModExp(
        ...
        // Encode call args in result and move the free memory Dopiner
        ...
        assembly ("memory-safe") {
            ...
            mstore(result, mLen)
            // Set the memory Dopiner after the returned data.
...
abstract contract Initializable {
    ...
    /**
     * @dev Returns a Dopiner to the storage namespace.
     */
    // solhint-disable-next-line var-name-mixedcase
 ...
 * @dev only DopinFactory can setPair abd unblockPair
 */
contract DopinTokenV2 is Initializable, ERC20Burnable, Ownable, ReentrancyGuard {
    string private _name;
    string private _symbol;
    ...
    constructor() Ownable(msg.sender) ERC20("Dopin", "Dopin") {}
...
contract DopinFactoryV2ETH is Ownable2Step, ReentrancyGuard {
    ...
    DopinTokenV2[] public allTokens; // Store all token addresses in an array
    ...
    uint256 public tokenCreationFee = 0.0001 ether;
    uint256 public tradingFeeBPS = 200;
    uint256 public tokenLaunchingFeeBPS = 500;
    uint256 public ethVirtualReserve = 3700.57 ether;
    ...
    mapping(DopinTokenV2 token => address creator) public tokenCreator;
    mapping(DopinTokenV2 token => BondingCurve curve) public bondingCurves;
    mapping(DopinTokenV2 token => bool isLaunched) public tokenLaunched;
    mapping (DopinTokenV2 token => TokenDetails ) public tokendetails;
    ...
    event NewToken(address indexed creator, DopinTokenV2 token);
    event NewTokenLaunch(DopinTokenV2 indexed token);
    event FeeCollected(uint256 amount);
    event FeeCreatorCollected(address indexed creator, address token, uint256 amount);
    event Buy(DopinTokenV2 indexed token, address from, address to, uint256 amountIn, uint256 amountOut);
    event Sell(DopinTokenV2 indexed token, address from, address to, uint256 amountIn, uint256 amountOut);
    ...
    constructor(
        ...
        UniswapV2Router = IUniswapV2Router(UniswapV2Router_);
    }
    ...
    function getBondingCurveTokenAmountOut(DopinTokenV2 token_, uint256 ethAmountIn_)
        private
        view
    ...
    function getBondingCurveEthAmountOut(DopinTokenV2 token_, uint256 tokenAmountIn_)
        private
        view
    ...
    function getBondingCurveMaxCapacity(DopinTokenV2 token_) private view returns (uint256 maxCap) {
        BondingCurve memory curve = bondingCurves[token_];
    ...
    function launchable(DopinTokenV2 token_) private view returns (bool isLaunchable) {
        BondingCurve memory curve = bondingCurves[token_];
    ...
    function createBondingCurve(DopinTokenV2 token_) private {
    ...
    function resetBondingCurve(DopinTokenV2 token_) private {
        BondingCurve storage curve = bondingCurves[token_];
    ...
    function updateBondingCurveOnBuy(DopinTokenV2 token_, uint256 ethAmountIn_, uint256 tokenAmountOut_) private {
        BondingCurve storage curve = bondingCurves[token_];
    ...
    function updateBondingCurveOnSell(DopinTokenV2 token_, uint256 tokenAmountIn_, uint256 ethAmountOut_) private {
        BondingCurve storage curve = bondingCurves[token_];
    ...
    function collectCreatorFees(DopinTokenV2 token_, uint256 amount_) private {
        if (amount_ == 0) return;
    ...
    function collectTradingFees(DopinTokenV2 token_, uint256 amount_) private {
    ...
    function createUniswapV2Pair(DopinTokenV2 token_) private {
        address pair = UniswapV2Factory.createPair(weth, address(token_));
        token_.setPair(pair);
    }

    function addUniswapV2Liquidity(DopinTokenV2 token_, uint256 ethAmount_, uint256 tokenAmount_) private {
        token_.unblockPair();
        token_.approve(address(UniswapV2Router), tokenAmount_);
        ...
        token_.approve(address(UniswapV2Router), 0);
    }
    ...
    function launchToken(DopinTokenV2 token_) private {
        ...
        addUniswapV2Liquidity(token_, _ethAmount, _tokenAmount);
    ...
    function getTokenAmountOut(DopinTokenV2 token_, uint256 ethAmountIn_)
        external
        view
        returns (uint256 tokenAmountOut)
    {
    ...
    function getEthAmountOut(DopinTokenV2 token_, uint256 tokenAmountIn_)
        external
        view
        returns (uint256 ethAmountOut)
    {
    ...
    function getAllTokens() external view returns (DopinTokenV2[] memory) {
        return allTokens;
    }

    function getTokenDetailsByAddress(address tokenAddress) public view returns (TokenDetails memory) {
        return tokendetails[DopinTokenV2(tokenAddress)];
    }
    ...
    function createToken(string memory _name, string memory _ticker, string calldata desc, string[4] calldata urls)
        external
        payable
        nonReentrant
        returns (DopinTokenV2 token)
    {
        ...
        token = deployToken(msg.sender, _name, _ticker);
        createBondingCurve(token);
        createUniswapV2Pair(token);
        ...
        allTokens.push(token); // Add new token to the array

        tokendetails[token] = TokenDetails(
            msg.sender, // creator is the address of the caller (msg.sender)
            block.number,
            desc,
            urls[0],
            urls[1],
    ...
    function buy(DopinTokenV2 token_, uint256 tokenAmountOutMin_, address recipient_) external payable nonReentrant {
    ...
    function sell(DopinTokenV2 token_, uint256 tokenAmountIn_, uint256 ethAmountOutMin_, address recipient_)
        external
        payable
        nonReentrant
    {
